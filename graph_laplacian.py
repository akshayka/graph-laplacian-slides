...
@app.cell(hide_code=True)
def graph_views(adjacency_matrix, data):
...
def _(L):
    ...
    return (eigenvalues, fiedler_eigenvector)


@app.cell(hide_code=True)
def _(adjacency_matrix, data, eigenvalues, n_neighbors):
    deck_stats = {
        "n_points": f"{data.shape[0]}",
        "k": f"{n_neighbors.value}",
        "n_edges": f"{int(adjacency_matrix.sum()) // 2:,}",
        "lambda2": f"{max(float(eigenvalues[1]), 0.0):.4f}",
    }
    return (deck_stats,)
...
@app.cell(hide_code=True)
def fiedler_plot(fiedler_eigenvector):
    plt.scatter(
...
@app.cell(hide_code=True)
def spectral_result(data, fiedler_eigenvector):
